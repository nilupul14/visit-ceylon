...
const AddShows = () => {
  ...
  const fetchNowPlayingMovies = async () => {
    ...
    try {
      ...
      if (data?.success && Array.isArray(data.movies) && data.movies.length > 0) {
        ...
        // api said success but no movies → use mock
  ...
  const handleSubmit = async () => {
    try {
      ...
      // server expects an array of date/time pairs
      ...
      <div className='overflow-x-auto pb-4'>
        <div className='group flex flex-wrap gap-4 mt-4 w-max'>
          {nowPlayingMovies.map(movie => {
            // poster can be absolute (mock) or relative (API)
            const poster =
            ...
            return (
              <div
                ...
                className={`relative max-w-40 cursor-pointer group-hover:not-hover:opacity-40 hover:-translate-y-1 transition duration-300 ${
                  selectedMovie === (movie._id || movie.id) ? 'opacity-100' : ''
                }`}
              ...
              >
                ...
                {/* your dummy data doesn’t have release_date anymore, so show tagline or fallback */}
                <p className='text-gray-400 text-sm'>
                  {movie.release_date || movie.tagline || 'Sri Lanka'}
                </p>
              </div>
            )
          })}
